...
class ViewportManager{
    constructor(viewportElement)
    {
        ...
        function onDrag()
        {
            ...
            {
                camera.rotateOnWorldAxis(new Vector3(0,1,0), diffX * 4);
                camera.rotateOnAxis(new Vector3(1,0,0), diffY * 4);
        ...
        function onPointerMove(event)
        {
            ...
            //camera.rotateOnAxis(new Vector3(1,0,0), diffY);
            //camera.getWorldDirection(forwardAxis)
            //forwardAxis.projectOnPlane(new Vector3(0, 1, 0));
        ...
        {
            const direction = new Vector3().copy(forwardAxis).normalize().multiplyScalar(velocity);
            if(moveForward) camera.position.add(direction);
            if(moveLeft) camera.position.sub(new Vector3().crossVectors(direction, new Vector3(0,1,0)));
            if(moveBackward) camera.position.sub(direction);
            if(moveRight) camera.position.add(new Vector3().crossVectors(direction, new Vector3(0,1,0)));
